package io.shubh.e_commver1.Splash.Presenter;
...
//checkForSystemUpdates();
  ...
  /*  public void checkIfAlreadyLoggedIn() {
        ...
        if (user!=null) {
            ...
            User user2 =new User(user.getUid() , user.getDisplayName() ,user.getEmail());
            StaticClassForGlobalInfo.user=user2;

            splashview.showToast("Logged in already");

            //1 means switch to Main/home activity
            splashview.SwitchActivity(1);

        } else {
            // No user is signed in.
            splashview.showProgreeBar(false);
            splashview.showToast("not logged in already,Please use Google sign in");

            //2 means Login Activity
            splashview.SwitchActivity(2);
        }
    }
    ...
    public void checkForSystemUpdates() {
        ...
        splashview.showProgreeBar(true);

        mInteractor.checkForSystemUpdate( new SplashInteractor.SeparateCallbackToPresnterForSystemUpdate(){
            @Override
            public void onFinishedCheckingSystemUpdate(boolean callbackResultOfTheCheck) {
                ...
                if(haveNetworkConnection()==true) {
                    if (callbackResultOfTheCheck == true) {
                        ...
                        splashview.showProgreeBar(false);
                        //system update is available so show the message with download procedure
                    } else {
                    ...
                    splashview.showProgreeBar(false);
                    splashview.ShowSnackBarWithAction("No Network!!" , "Retry");
                }
            }
        });
    }*/
